...
1. 選「遠端雙人 · 公開大廳」，所有玩家共用房間 1、2、3，每房最多兩人。
2. 在空房按「開房等待 · P1」，保持頁面開啟，等待其他玩家參賽。
3. 其他玩家直接在大廳按「加入對戰 · P2」，不需傳送邀請連結。大廳每 10 秒自動更新，也能手動重新整理；已滿或無法確認的房間不可加入。房主離開後房間釋出，服務可能需短暫時間更新。
4. P1 用角色卡選將，P2 用「P2 武將」選單選將。兩邊都用空白鍵發射、1–8 通用技、Q 專屬技、P 暫停。也能按自己的技能按鈕。
5. 雙方選好武將後按「準備好了」，兩人都準備好才進入蓄力。等待時可以取消準備；更換任一武將會取消雙方準備。雙方各自鎖定發射時機，兩人都鎖定才開戰。每場結束後需要重新準備。
...
大廳使用三個固定 PeerServer ID，利用服務端 ID 唯一性限制最多三個公開房間；同時搶開房只有一人成功。查詢連線不占玩家席位。此限制適用本遊戲正常客戶端，不是具帳號權限的伺服器配額。

這是公開對戰原型，房主的瀏覽器是權威端；沒有競技遊戲的獨立防作弊伺服器。房主切換分頁會暫停對戰。斷線後需重新建立／加入房間。
...
`python3 scripts/lobby-smoke.py`：真實公開大廳測試，檢查三間房上限、重複開房被拒絕、房間釋出與手機版面。
