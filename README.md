...
開始蓄力會自動進入戰鬥畫面；也可先按「進入戰鬥畫面」。桌機技能在戰場旁，手機直向在下方，全部 9 招保留在視窗內。遠端只顯示自己的技能；同機雙人窄螢幕可切換 P1／P2 技能面板。按「招式說明」查看完整效果，按「返回頁面」或 Escape 先暫停再離開；重新選將則回到設定頁。音量與靜音仍可直接操作。

`python3 scripts/layout-smoke.py` 驗證五種視窗尺寸、技能可見性、離開暫停、返回與面板切換。
